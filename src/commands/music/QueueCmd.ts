...
export default class QueueCommand extends Command {
    ...
    public async run(msg: CommandoMessage): Promise<Message|Message[]> {
        ...
        msgg.createReactionCollector(filter, {
            time: 3e4
        })
            .on("collect", (e: MessageReaction) => {
                const collect = e.emoji.name;
                if (collect === emoji[0]) {
                    index--;
                    void msgg.reactions.resolve(emoji[0]).users.remove(msg.author);
                }
                if (collect === emoji[1]) {
                    index++;
                    void msgg.reactions.resolve(emoji[1]).users.remove(msg.author);
                }
            ...
            .on("end", () => {
                void msgg.reactions.removeAll();
            });
    }

    public arr<T>(array: T[], amount: number): T[][] {
        const arr = [];
